...
mdc_correctness_simple(const char *mpool, const struct hse_params *params)
{
    ...
    /* 2. Open the mpool */
    err = mpool_open2(mpool, params, 0, &mp);
    if (err) {
        original_err = err;
    ...
    /* 3. Create an MDC */
    err = mpool_mdc_alloc2(mp, MDC_TEST_MAGIC, 1 << 20, mclass, &oid[0], &oid[1]);
    if (err) {
        original_err = err;
        merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
    ...
    err = mpool_mdc_abort2(mp, oid[0], oid[1]);
    if (err) {
        original_err = err;
        merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
    ...
    err = mpool_mdc_commit2(mp, oid[0], oid[1]);
    if (err) {
        original_err = err;
        merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
    ...
    /* 4. Open the MDC */
    err = mpool_mdc_open2(mp, oid[0], oid[1], &mdc);
    if (err) {
        original_err = err;
        merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
    ...
    err = mpool_mdc_delete2(mp, oid[0], oid[1]);
    if (err) {
        original_err = err;
        merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
...
mdc_correctness_multi_reader_single_app(const char *mpool, const struct hse_params *params)
{
    ...
    /* 4. Open MDC */
    err = mpool_mdc_open2(mp, oid[0], oid[1], &mdc[0]);
    if (err) {
        original_err = err;
        merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
        fprintf(stderr, "%s.%d: Unable to open MDC: %s\n", __func__, __LINE__, errbuf);
    ...
    for (i = 0; i < BUF_CNT; i++) {
        ...
        err = mpool_mdc_append2(mdc[0], buf, BUF_SIZE, true);
        if (err) {
            merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
            fprintf(stderr, "%s.%d: Unable to append to MDC: %s\n",
                __func__, __LINE__, errbuf);
            mpool_mdc_close2(mdc[0]);
            goto destroy_mdc;
        }
    }
    ...
    for (i = 0; i < BUF_CNT; i++) {
        ...
        err = mpool_mdc_read2(mdc[0], buf_in, BUF_SIZE, &read_len);
        if (err) {
            original_err = err;
            merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
    ...
    err = mpool_mdc_open2(mp, oid[0], oid[1], &mdc[1]);
    if (err) {
        original_err = err;
        merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
        fprintf(stderr, "%s.%d: Unable to open MDC: %s\n", __func__, __LINE__, errbuf);
    ...
    for (i = 0; i < BUF_CNT; i++) {
        ...
        err = mpool_mdc_read2(mdc[1], buf_in, BUF_SIZE, &read_len);
        if (err) {
            original_err = err;
            merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
            fprintf(stderr, "%s.%d: Unable to read from MDC: %s\n",
...
mdc_correctness_reader_then_writer(const char *mpool, const struct hse_params *params)
{
    ...
    for (i = 0; i < BUF_CNT; i++) {
        ...
        err = mpool_mdc_append2(mdc, buf, BUF_SIZE, true);
        if (err) {
            merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
            fprintf(stderr, "%s.%d: Unable to append to MDC: %s\n",
                __func__, __LINE__, errbuf);
    ...
    for (i = 0; i < BUF_CNT; i++) {
        ...
        err = mpool_mdc_read2(mdc, buf_in, BUF_SIZE, &read_len);
        if (err) {
            merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
            fprintf(stderr, "%s.%d: Unable to read from MDC: %s\n",
                __func__, __LINE__, errbuf);
...
mdc_correctness_multi_mdc(const char *mpool, const struct hse_params *params)
{
	...
	for (i = 0; i < mdc_cnt; i++) {
        err = mpool_mdc_alloc2(mp, MDC_TEST_MAGIC + i, 1 << 20, mclass, &oid[i].oid[0], &oid[i].oid[1]);
		if (err) {
			original_err = err;
			merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
			fprintf(stderr, "%s.%d: Unable to alloc mdc: %s\n",
		...
		err = mpool_mdc_commit2(mp, oid[i].oid[0], oid[i].oid[1]);
		if (err) {
			original_err = err;
			merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
			fprintf(stderr, "%s.%d: Unable to commit mdc: %s\n",
	...
	for (i = 0; i < mdc_cnt; i++) {
		err = mpool_mdc_open2(mp, oid[i].oid[0], oid[i].oid[1], &mdc[i]);
		if (err) {
			original_err = err;
			merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
	...
	for (i = 0; i < mdc_cnt; i++) {
		...
		for (j = 0; j < BUF_CNT; j++) {
			...
			err = mpool_mdc_append2(mdc[i], buf, BUF_SIZE, true);
			if (err) {
				merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
				fprintf(stderr, "%s.%d: Unable to append to MDC: %s\n",
					__func__, __LINE__, errbuf);
	...
	for (j = 0; j < BUF_CNT; j++) {
		for (i = 0; i < mdc_cnt; i++) {
			...
			err = mpool_mdc_read2(mdc[i], buf_in, BUF_SIZE, &read_len);
			if (err) {
				merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
				fprintf(stderr, "%s.%d: Unable to read from MDC: %s\n",
					__func__, __LINE__, errbuf);
...
close_mdcs:
	for (i = 0; i < mdc_cnt; i++)
		mpool_mdc_close2(mdc[i]);

	/* Destroy the MDCs */
destroy_mdcs:
	for (i = 0; i < mdc_cnt; i++) {
		err = mpool_mdc_delete2(mp, oid[i].oid[0], oid[i].oid[1]);
		if (err) {
			if (!original_err)
				original_err = err;
			merr_strinfo(err, errbuf, ERROR_BUFFER_SIZE, NULL);
